...
<head>
    ...
    <title>Employee Monthly Attendance Sheet</title>
    <style>
        table {
            width: 90%;
...
<body>
    <h2>Employee Daily Attendance for August 2024</h2>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>Employee Name</th>
                    <?php
                    // Generate day headers for the month
                    $month = 8; // August
                    $year = 2024;
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        echo "<thead><tr><th>Employee ID</th><th>Employee Name</th><th>Total Present Days</th><th>Total Absent Days</th></tr></thead>";
        echo "<tbody>";

        // Calculate totals and display attendance report
        foreach ($employees as $employee) {
            $total_present = 0;
            $total_absent = 0;
            ...
            for ($day = 1; $day <= $days_in_month; $day++) {
                $attendance_key = "{$employee['id']}_{$day}";
                if (isset($_POST['attendance'][$attendance_key]) && $_POST['attendance'][$attendance_key] === 'Present') {
                    $total_present++;
                } else {
                    $total_absent++;
                }
