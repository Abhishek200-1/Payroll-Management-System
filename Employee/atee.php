...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        echo "<thead><tr><th>Employee ID</th><th>Employee Name</th>";
        ...
        // Display attendance status for each employee
        foreach ($employees as $employee) {
            echo "<tr>";
            echo "<td>{$employee['id']}</td>";
            echo "<td>{$employee['name']}</td>";
            ...
            for ($day = 1; $day <= $days_in_month; $day++) {
                $attendance_key = "{$employee['id']}_{$day}";
                $status = isset($_POST['attendance'][$attendance_key]) ? 'Present' : 'Absent';
                echo "<td>$status</td>";
            }
